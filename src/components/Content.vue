...
function contextMenuClick(args) {
  ...
  if (
    ...
    if (
      args.item.iconCss.indexOf("e-adhocs") > -1 &&
      bpmnShape.activity &&
    ...
    if (
      ...
      if (node.shape !== "DataObject" && node.shape !== "Gateway") {
        ...
        if (
          ...
          if (
            diagram.selectedItems.nodes[0].shape.shape === "DataObject" ||
            node.shape === "Activity" ||
            diagram.selectedItems.nodes[0].shape.shape === "Gateway"
          ) {
            hiddenId.splice(hiddenId.indexOf(item.id), 1);
          }
        }
